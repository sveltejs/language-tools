	...
	function extractLocations(input) {
		...
		while (line = lines.shift()) {
			...
			if (/^\s*\d/.test(line)) {
				...
				const endSpan = () => {
					if (offsets.length) {
						locations.set(currentId, { line: source_line, start: start, offsets:offsets });
					}
					offset = 0
					offsets = []
				}
		...
		(solo ? it.only : it)(dir, () => {
			...
			for (let [id, span] of input.locations.entries()) {
				...
				for (var off = 0; off < span.offsets.length; off++) {
					...
					let { line: actual_line, column: decoded_col } = decoder.originalPositionFor({ line: input_line, column: input_col - 1 })
					...
					if (actual_line == actual_result_line) {
						if (actual_result_line[actual_col - 1] == " ") {
							...
							actual_result[actual_col - 1] = `${Math.min((actual_result[actual_col - 1] << 0) + 1, 9)}`
						}
					} else {
