///<reference types="svelte" />
;function $$render() {
async () => {if(true){
 { __sveltets_2_createComponentAny({  "prop1":1,});}
}
 { const $$_tnenopmoc_etlevs0C = __sveltets_2_ensureComponent(testComponent); new $$_tnenopmoc_etlevs0C({ target: __sveltets_2_any(), props: {   children:() => { return __sveltets_2_any(0); },"propa":5,}});
     { svelteHTML.createElement("h1", {});  }
 }
 { svelteHTML.createElement("svelte:window", {   "on:click":e => {},});}
 { svelteHTML.createElement("svelte:body", {   "on:click":e => {},});}
 { svelteHTML.createElement("svelte:head", {});
     { svelteHTML.createElement("h1", {});  }
 }
 { svelteHTML.createElement("svelte:options", {});}
 { svelteHTML.createElement("svelte:fragment", {});}
 { svelteHTML.createElement("svelte:document", {    "foo":`bar`,"on:click":e => {},});}};
return { props: /** @type {Record<string, never>} */ ({}), exports: {}, bindings: "", slots: {}, events: {} }}
const Input__SvelteComponent_ = __sveltets_2_isomorphic_component(__sveltets_2_partial(__sveltets_2_with_any_event($$render())));
/*Ωignore_startΩ*/type Input__SvelteComponent_ = InstanceType<typeof Input__SvelteComponent_>;
/*Ωignore_endΩ*/export default Input__SvelteComponent_;