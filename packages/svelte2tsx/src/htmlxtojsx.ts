...
export function convertHtmlxToJsx(str: MagicString, ast: Node, onWalk: Walker = null, onLeave: Walker = null) {
    ...
    const handleAwait = (awaitBlock: Node) => {
        ...
        if (!awaitBlock.catch.skip) {
            ...
            // eslint-disable-next-line max-len
            const errorStart = awaitBlock.error ? htmlx.indexOf(awaitBlock.error, catchSymbolEnd) : catchSymbolEnd;
