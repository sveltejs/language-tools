...
export class CodeActionsProviderImpl implements CodeActionsProvider {
    ...
    private checkIndentLeftover(change: TextDocumentEdit, document: Document) {
        ...
        for (let i = 0; i < orderedByStart.length; i++) {
            ...
            // current ends at the start of a line and the next edit also starts at the same line with only whitespace in between
            if (
                current.range.end.character === 0 &&
                current.range.end.line === edit.range.start.line &&
                !current.newText &&
                !edit.newText
            ) {
                const inBetween = document.getText({
                    start: current.range.end,
                    end: edit.range.start
                });
                if (!inBetween.trim()) {
                    edit.range.start = { line: current.range.end.line, character: 0 };
                    current.range.end = edit.range.end;
                    continue;
                }
            }
